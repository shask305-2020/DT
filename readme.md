...
Вообще, на сервер надо выкладывать РАБОЧИЙ проект, эксперты не будут разбираться почему ваш код не запускается. Поэтому предыдущие команды можно выполнять периодически, по мере реализации каких-то законченных действий.
